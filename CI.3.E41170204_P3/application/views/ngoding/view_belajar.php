<html>
<html>
<head>
	<title>Cara Membuat View Pada CodeIgniter | MalasNgoding.com</title>
</head>
<body>
	<h2><?php echo $judul; ?></h2>
	<h3><?php echo $tutorial; ?></h3>
	<?php 
	echo heading("Helper HTML CodeIgniter | MalasNgoding.com",1);br();
	echo heading("Helper HTML CodeIgniter | MalasNgoding.com",2);br(2);
	echo heading("Helper HTML CodeIgniter | MalasNgoding.com",3);br(3);
	echo heading("Helper HTML CodeIgniter | MalasNgoding.com",4);
	echo heading("Helper HTML CodeIgniter | MalasNgoding.com",5);
	echo heading("Helper HTML CodeIgniter | MalasNgoding.com",6);
	?>
</body>
</html>